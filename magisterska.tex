...
\povzetek
Magistrsko delo obravnava razvoj in razvojno testiranje komunikacijskega modula za družino pametnih števcev električne energije IE.X podjetja Iskraemeco d.d. Uvodoma smo predstavili pametni števec in vlogo mobilnih omrežij pri njegovem daljinskem odčitavanju ter primerjali mobilne tehnologije, ki jih podjetje uporablja. Izhajajoč iz zahtev internega standarda P* smo določili električne in mehanske lastnosti komunikacijskega modula ter opisali razvoj njegovih glavnih funkcijskih sklopov. V nadaljevanju je predstavljeno razvojno testiranje modula, v okviru katerega smo avtomatizirali postopek za preverjanje funkcije varnega ugašanja modula ter na podlagi rezultatov predlagali možnosti za nadaljnje izboljšave.
...
\abstract
This thesis addresses the development and pre-production testing of a communication module for the IE.X family of smart electricity meters manufactured by Iskraemeco d.d. In the introduction, we present the smart meter and the role of mobile networks in its remote reading, and compare the mobile technologies used by the company. Based on the requirements of the internal P* standard, we define the electrical and mechanical properties of the communication module and describe the development of its main functional blocks. The thesis then presents the development testing of the module. As part of this testing, we automate the procedure for verifying the safe shutdown function and, based on the results, propose options for further improvements.
...
\par Cilj magistrske naloge je opisati razvoj in razvojno testiranje komunikacijskega modula za družino pametnih števcev IE.X podjetja Iskraemeco d.d.
\begin{figure}[h]
    \centering
    \includesvg[
    width=0.8\columnwidth,inkscapelatex=false
...
\end{figure}
Pin \texttt{GPIO6} na modulu ima definiran interni pull-up upor. Tekom delovanja modul spremlja pin, ter ko se naredi negativni prehod, konča z vpisovanjem v bliskovni pomnilnik ter se sam ugasne. Vezje, ki ob padcu napetosti naredi negativni prehod na \texttt{GPIO6} prikazuje slika \ref{fastshutdovn_vezje}.
...
Načrtovano tiskano vezje smo naročili ter ga opremili v proizvodnji podjetja. Izdelek je prikazan na sliki \ref{modul} Sledi testiranje, pri katerem smo dali poudarek na funkcijske sklope, opisane v zgornjih poglavjih. Pri testiranju smo si pomagali z interno razvito vgrajeno programsko opremo \textit{Advanced Test Bench}, namenjeno razvojnemu in proizvodnemu testiranju. Omogoča nam testiranje strojne opreme in uporabo določenih delov periferije (GPIO, UART, SPI..) mikrokrmilnika na števcu. 
\begin{figure}[!h]
    \centering
...
\end{figure}
\par
Modul, opremljen s kartico SIM, se po ustrezni konfiguraciji APN-ja in PDP-konteksta uspešno poveže v mobilno omrežje ter pridobi naslov IP. Po vzpostavitvi povezave modul preklopimo v način delovanja GPRS in prek konzole svojega osebnega računalnika izvajamo ukaz ping proti modemu ter opazujemo napajalno napetost modema. Na sliki \ref{GPRS_power_dip} lahko vidimo, da se napetost ob t.i. TX burstu posede, vendar ne pod kritično mejo, ki je definirana v podatkovnem listu modema. Opazili smo, da je tok iz števca v modul manjši od tokovne limite \SI{1}{\ampere}, kar bo potrebno nadalje raziskati, verjetno pa je manjši tok posledica feritnih dušilk na števcu in modulu, ki so namenjena zmanjševanju visokofrekvenčnih motenj na napajanju. Slika prikazuje porabo toka ter posedanje napetosti na glavni napajalni liniji. 
\begin{figure}[!h]
    \centering
...
\end{figure}
\par
Preverili smo tudi delovanje funkcije \textit{last gasp}. Ultra kondenzator na števcu je napolnjen na napetost \SI{2}{\volt}, ob izpadu napajanja pa s pomočjo pretvornika navzgor napetost na ultra kondenzatorju dvignemo na 4V, blokovno shemo prikazuje slika \ref{blok_shema_napajanje_stevec}. Pretvornik navzgor svojo funkcijo opravlja dokler je napetost na ultra kondenzatorju vsaj \SI{0,5}{\volt} napetosti. Števec smo nastavili tako, da \SI{30}{\second} po izpadu napajanja pošlje sporočilo na izbrani naslov IP ter vrata preko protokola UDP. Na računalniku smo spremljali komunikacijo na določenih vratih ter uspešno prejeli števčevo sporočilo, ob tem pa smo na osciloskopu spremljali napetost na števčnem kondenzatorju. Rezultate prikazuje slika \ref{last_gasp}, iz katere lahko odčitamo, da je po poslanem sporočilu na ultra kondenzatorju še dovolj energije, da se števec lahko varno ugasne.
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.9\columnwidth,trim=0cm 0.4cm 0.0cm 0.8cm, clip]{Slike/last gasp.png}
    \caption{Praznenje ultra kondenzatorja na števcu po izpadu napajanja.}
    \label{last_gasp}
    \vspace{10pt}
    \small
    \begin{tabular}{@{}ll@{}}
        Opis signalov na oscilogramu:\\
         Rumena: &Napetost na ultra kondenzatorju \\
...
Ob povezavi v omrežju smo lahko testirali funkcijo \textit{fastshutdown}. Modul smo priklopili na nastavljiv napetostni izvir ter pomerili napetosti, pri katerih se zgodi preklop na NCP308. Histerezo prikazuje slika \ref{fig:fastshutdown_detektor_oscilogram}, s pomočjo markerjev lahko odčitamo, da sta napetosti $U_{th-}$ in $U_{th+}$ \SI{3.485}{\volt} in \SI{3.818}{\volt}, kar se ujema z izračuni v prejšnjem poglavju, manjša odstopanja pa predpisujemo tolerancam komponent.

\begin{figure}[!h]
    \centering
    \includegraphics[width=0.9\columnwidth,trim=0cm 0.4cm 0.0cm 0.8cm, clip]{Slike/fastshutdown_detektor.png}
    \caption{Histereza preklopa
nadzornika napetosti NCP308}
...
V okviru razvoja smo obravnavali ključne funkcijske sklope modula. Razložili smo diskretni pretvornik napetostnih nivojev med \SI{3,3}{\volt} in \SI{1,8}{\volt} logiko, vezje za samodejni vklop modula z zakasnjenim generiranjem negativnega pulza na pinu \texttt{PWRKEY}, napajalni del s tokovno limito, ki preprečuje prevelik odjem toka iz števca. Pozornost smo namenili zanesljivemu delovanju ob izpadu napajanja ter varnemu ugašanju modema, skupaj s proizvajalcem uvedli funkcijo hitrega izklopa (fastshutdown) ter strojni prožilnik te funkcije skupaj z dodatno zalogo energije v kondenzatorju, ki prepreči okvaro bliskovnega pomnilnika ob nenadni prekinitvi napajanja. Na koncu smo modul realizirali na 4-slojnem tiskanem vezju, ki smo ga načrtovali v skladu s pravili EMC ter zahtevami za proizvodno testiranje.
 ...
 Razvili smo avtomatiziran postopek za testiranje funkcije \textit{fastshutdown}, ki nam bo pomagal tudi v prihodnosti pri testiranju robnih pogojev, ki se pojavljajo ob nenadnih izklopih. S tem postopkom smo ovrednotili izboljšavo funkcije \textit{fastshutdown} po nadgradnji vgrajene programske opreme. Glede na rezultate testiranja funkcije \textit{fastshutdown} bodo potrebne ponovne meritve, ki nam bodo povedale, ali je shranjevanje dodatne energije v kondenzatorju EDLC sploh potrebno.
